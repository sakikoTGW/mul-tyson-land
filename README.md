...
> 给定平面闭域 $\Omega$、第一面旗 $p_1$、边界故事 $\tau$，反求其余旗位与半径，并判定解空间（无解 / 唯一 / 有限 / 无穷）。下文给出主问题定义、封口规格、核心定理与证明；仓库内脚本为数值复验，**非证明链**。

---

## 零、问题怎么说

想象你有一块奇形怪状的地皮——可能是海星形的，也可能是中间有个湖的环形地块。你在这块地皮的某个位置站定，插下**第一面旗子**（位置给定，管辖半径归一为 $r_1=1$）。

你要做的不是直接画地盘，而是先在这块地的其他位置也插上旗子。每面旗有自己的**管辖半径** $r_i>0$：半径越大，同样距离下性价比越低，势力范围在几何上能「画」得越大。然后整块地按**乘性泰森**规则切成一片片势力区：每寸土地归**性价比**

$$
\varphi_i(x)=\frac{\|x-p_i\|}{r_i}
$$

**最小**的那面旗管。两旗势力相遇处，分界线一般是**阿波罗尼奥斯圆**（仅当两旗半径相等时才退化成直线）。

有两条硬规矩：

1. **G1 覆盖**：整块地要分得刚刚好，不能有空出来的地方（$V_1\cup\cdots\cup V_N=\Omega$）；
2. **边界故事 $\mathcal B$**：地皮边界每条边（除分界点外）应归哪面旗，由 $\tau$ 事先指定；实际划分必须与此一致。

最核心的是**反问题**：第一面旗给定，但其余旗插在哪里、每面半径设多大，都要从地皮形状与边界故事里**算**出来。有些形状可能只有唯一一组插法才能完美分完；有些可能有多种分法；有些可能根本没有完美解。数学上要问的是：解集 $\mathfrak{Sol}$ 为空、唯一、有限离散，还是无穷多解（并给出参数化闭式）。
...
本节摘自主文稿 `symmetric_sector_theorems.md`（本地，未入库），给出宽题封口与收窄记录；后文定理 1.1 即主文稿**定理 M★** 在路线 A、$\mathcal B^\*$ 下的表述。
...
### W.2 收窄记录（特设 $\Sigma$，不得升格为宽题解）

| 编号 | 特设 | 与宽题 W 的关系 | 状态 |
|------|------|-----------------|------|
| $\Sigma_2$ | 路线 A：钉 $M\in\Gamma_{12}$，$\lambda$ 由 (1.3) 定 | 单钉外特征下的子题 | 充分性片段 ✅ |
| $\Sigma_5$ | 强规范 $\mathcal B^\*$：渗漏边无 $V_2$ 严格内点 | 推论 M 的规范 | ✅ |
| $\Sigma_6$ | $n_{\min}$ 相对 $\mathcal B^\*$ 定义 | W 的子目标 | 定理 M★ ✅ |
| $\Sigma_9$ | 扇形可接受解 + (I)(II)(III) 故事 | 扇形子题完整三分 | ✅ 第八部分 |
| $\Sigma_{10}$ | 乘性泰森 $\varphi_i=d/r_i$（非幂图） | 主模型 | ✅ |

### W.6 乘性泰森主模型（$\mathfrak{Sol}^{\mathrm{mul}}$）

胞间分界线：$\varphi_i=\varphi_j$ 为**阿波罗尼奥斯圆**（$r_i=r_j$ 时退化为直线）。$\partial\Omega$ 的直边、光滑弧是**域裁剪**，一般**不能**与某 $\Gamma_{ij}$ 重合——这与 Laguerre **幂图**（胞间为直线）不同，幂图仅作对照。
...
宽题 W 的「存在配置 $\mathcal S$ 满足 $\mathcal G\cup\mathcal B$」**等价于** $\mathfrak{Sol}^W\neq\varnothing$，其中 $\mathfrak{Sol}^W$ 为对有限 $N$ 与有限拓扑分支 $\mathfrak g$ 上 $\mathfrak{Sol}^{\mathrm{mul,full}}$ 的并集投影。纯解析判定 = 对每个分支判定半代数集是否为空（Tarski–Seidenberg / Gröbner），**不用**数值迭代进证明链。
...
### 定理 M★（$\Sigma+\mathcal B^\*$ 下 $n_{\min}$ 二分 —— 即下文定理 1.1）
...
**判定程序**（固定 $N=2$ 共线时）：先数 $\mathrm{dof}$；再数钉点 $|\Pi|$ 与额外独立等式；若 $|\Pi|<\mathrm{dof}$ 则解空间必为正维；若 $|\Pi|=\mathrm{dof}$ 则查方程相容性（双钉常失败）；若需唯一，加第 $\mathrm{dof}$ 个独立等式（$\eta$、固定半径等）。

**与 $n_{\min}$ 的分工**：$n_{\min}$ 回答「最少几面旗」；本节回答「固定 $N$ 下要钉几个点才有有限/唯一」。二者正交。

---

## 第六部分　定理 1.1（最少旗数，秩 3）
...
由 $p_3=p_2$、$r_3=\lambda$，第二式化为 $d(X,p_2)<d(X,p_2)$，恒假。故**全局无旗 2 严格内点**，渗漏边上的旗 2 开段消失，$\mathcal B^\*$ 成立。钉点 $M\in\Gamma_{12}$ 仍满足 (1.3)。故 $n=3$ 可行，$n_{\min}\le 3$. 结合下界，$m_{\mathrm{eff}}\ge 1\Rightarrow n_{\min}=3$.

**$m_{\mathrm{eff}}=0$**　无渗漏，两旗已够，$n_{\min}=2$. 证毕.

### 注（秩 3 共点构造）

增旗的直接需求是在渗漏边 $e$ 上压制旗 2 的严格占优。几何分离路线（不同位置的 $p_3$）在 $m_{\mathrm{eff}}\ge 2$ 时可能需要 $n_{\min}^{\mathrm{geo}}=4$（顶/底守卫不相容）。秩 3 路线选择**同点复制** $(p_2,\lambda)$，使第三列比较退化，从谓词层面取消「旗 2 严格内点」，而不移动 $M\in\Gamma_{12}$。这是 $\mathcal B^\*$ 下达到 $n=3$ 的**最小**代数增旗方式。扇形上 $V_1\cup V_2=\Omega$ 已由引理 2.3、5.1(iv) 成立；增旗机制来自矩形/L 边界的渗漏审计，而非补未覆盖区域。
...
| 边界 $\mathcal B^\*$ | 顶/底边 $x>w$ 段渗漏 $\Rightarrow m_{\mathrm{eff}}=2$，$x_Q\approx0.92$，渗漏区间 $\approx(0.55,1.95)$ |
| $n_{\min}$ | 秩 3：3；几何分离：4 |
...
输入 $(\Omega,p_1,\tau)$ 经下列步骤收束为可判定规格；各步均为前述定理的拼接。
...
| 渗漏边存在 | $m_{\mathrm{eff}}\ge1$，两旗不满足 $\mathcal B^\*$ | 定理 1.1，秩 3，$n_{\min}=3$ |
| 矩形/L/扇形 | 仅代入 $\partial\Omega$ 算 $m_{\mathrm{eff}}$ 与 dof | 不另立形状定理 |

---

## 第十六部分　路线 C 守卫（$m_{\mathrm{eff}}\ge2$）
